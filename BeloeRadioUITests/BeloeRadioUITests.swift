...
class BeloRadioUITests: XCTestCase {

    let app = XCUIApplication()
    let stations = XCUIApplication().cells
    let hamburgerMenu = XCUIApplication().navigationBars["Beloe Radio"].buttons["icon hamburger"]
    ...
    func assertHamburgerContent() {
        XCTAssertTrue(app.staticTexts["Created by: Mattehew Fecher"].exists)
    }
    
    func assertAboutContent() {
        XCTAssertTrue(app.buttons["email me"].exists)
        XCTAssertTrue(app.buttons["Beloe-Radio.com"].exists)
    }
    ...
    func testMainStationsView() {
        ...
        app.navigationBars["Beloe Radio"].buttons["btn nowPlaying"].tap()
        waitForStationToLoad()
